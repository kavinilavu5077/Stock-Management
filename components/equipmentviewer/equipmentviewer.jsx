...
const EquipmentViewer = ({ equipmentNumber, category, onCancel }) => {
  ...
  useEffect(() => {
    const fetchColumnNames = async () => {
      const response = await axios.get(`http://localhost:8800/api/columnNames`, {
        params: { tableName: tableNames[currentTableIndex] },
      });
  ...
  }, [currentTableIndex, isInsertMode, tableNames, columnNames.length]);
  
  useEffect(() => {
    if (tableNames[currentTableIndex]) {
      setIsDataGrid(tableNames[currentTableIndex].includes("bearing") || tableNames[currentTableIndex].includes("vibration"));
    } else {
      setIsDataGrid(false);
    }
  }, [currentTableIndex, tableNames]);
  ...
  useEffect(() => {
    fetchTableData();
  }, [currentTableIndex, tableNames, fetchTableData]);
  ...
  const handleDelete = async (rowIndex) => {
    ...
    if (rowData.id) {
      await axios.delete("http://localhost:8800/api/tableData", {
        data: {
          tableName: tableNames[currentTableIndex],
          id: rowData.id,
        },
      });
    }
  ...
  const renderInsertMode = () => {
    const currentConfig = equipmentConfiguration[category]?.find(
      (config) => `${equipmentNumber}_${config.name}` === tableNames[currentTableIndex]
    );
...
const renderDataGrid = () => {
  if (!tableNames[currentTableIndex] || (!tableNames[currentTableIndex].includes("bearing") && !tableNames[currentTableIndex].includes("vibration"))) {
    return null;
  }
        ...
        <div className={styles.container}>
          <h2 className={styles.heading}>
            {tableNames[currentTableIndex] &&
              convertTableNameToDisplayName(tableNames[currentTableIndex])}
          </h2>
